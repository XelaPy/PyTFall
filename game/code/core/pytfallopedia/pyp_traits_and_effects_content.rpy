        ...
        vbox:
            ...
            hbox:
                ...
                viewport:
                    ...
                    text "Technically close to traits, Classes are characters occupations. They define the jobs character can and willing to do. They always give certain bonuses to stats and skills useful for the corresponding jobs. \n\n At the moment Class cannot be changed after character creation. It's possible to have two Classes, in which case each of the classes gives only a half of its bonuses. \n\n"
                    vbox:
                    ...
                    vbox:
                        label "Maid"
                        text "A universal service class who can perform many tasks with high efficiency, but also requires much more training. Capable to replace Cleaners and Barmaids."
                    vbox:
                        label "Manager"
...
screen pyp_fixed_traits():
    ...
    fixed:
        ...
        vbox:
            ...
            text "A number of traits should always present no matter the character. They define the core characteristics, and cannot be changed after the character creation. \n\n All fixed traits provide lots of bonuses and penalties of different kind, so they are very important for defying a character."
            hbox:
                spacing 2
                viewport:
                    draggable 1
                    ...
                    vbox:
                        ...
                        text "Defines character personality, heavily affects all dialogues and decides what the character likes and dislikes in general. The used personalities are based on a number of Japanese archetypes, such as Tsundere, Kuudere, etc. The list of races can be found in content/db/traits/traits_personality.json file in the game folders. Personalities have big icons in the top left corner of the character profiles."
                    vbox:
                        label "Body type (female only at the moment)"
                        text "General body constitution. While in real life one can change it by visiting gym or eating too much fastfood, we are limited by pictures base. Therefore body type is set in stone. Can be Lolita, Slim, Chubby, Athletic."
                    vbox:
                        label "Boobs size (female only of course)"
                        text "The size of character boobs, something very important in a brothel game. Can be Abnormally Large, Big, Average and Small."
                    vbox:
                        label "Race"
                        text "Used in the fantasy meaning of the word, such as Elf, Undead, Android, etc. Sometimes provides a number of unique effects. Each race has a unique icon. The list of races can be found in content/db/traits/traits_races.json file in the game folders."
                vbox:
                    frame:
                        yalign .5
                        background Frame("content/gfx/frame/mes11.webp", 10, 10)
            ...
            hbox:
                ...
                viewport:
                    ...
                    vbox:
                        ...
                        text "The base elemental traits are elements themselves. The full list is: Fire, Water, Air, Earth, Electricity, Ice, Light, Darkness. Each of them buffs some elemental attacks and defences and reduces others, for instance Fire gives +25% to fire damage and 15% to fire and air defences, but reduces ice spells damage by 25% and lowers water defence by 30%. One character can even have all elements at once, but since they all have pros and cons it's highly unwise, as it gives no positive effect. Instead it's better to focus on a few elements."
                    vbox:
                        label "General Elemental Traits"
                        text "Unlike base elemental traits, general traits may not present at all, or be temporally, but they affect elements in one way or another: Immunity traits protect against element completely, Absorption traits not only protect but also restore character health if attacked by corresponding element. Others provide bonuses to attack or defence of a certain element. Some of these traits can be obtained by equipping an item, others available only during character creation. Some can only be used by monsters."
                    vbox:
                        label "Neutral Element"
                        text "When a character has no base elemental traits, he/she automatically gets Neutral trait. It provides a solid defence against most elements, but gives big penalties to elemental damage. Making it a good choice only for characters who don't rely on elemental damage of any kind."
                    vbox:
...
screen pyp_effects():
    ...
    fixed:
        ...
        vbox:
            ...
            text "Effects are unique conditions with special logic. Some of them accompany a certain trait, others are independent. The difference between trait and effect is that trait can do a limited number of things (like change max stat), while effect can do pretty much anything and even have have special conditions of obtaining and removing it. The list of existing effects is below. Most of them are female-only at the moment."
            ...
            viewport:
                ...
                vbox:
                    label "Poisoned"
                    text "Health decreases over time until effect is removed by a potion"
                vbox:
                ...
                vbox:
                    label "Sibling"
                    text "Tied to certain trait. If disposition is low enough, it gradually increases over time." 
                vbox:
                    label "Assertive/Diffident"
                    text "Tied to certain personality traits. If character stat is high/low enough, it slowly decreases/increases over time." 
                vbox:
                ...
                vbox:
                    label "Elation"
                    text "Appears if joy is maxed out for some time. 10% chance of +1AP in the beginning of the day." 
                vbox:
                    label "Drinker"
