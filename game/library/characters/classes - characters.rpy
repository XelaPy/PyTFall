...
init -9 python:
    ...
    class PytCharacter(_object):
        ...
        def auto_buy(self, item=None, amount=1, equip=False):
            ...
            if dice(80):
                ...
                for i in newpool:
                    if i not in hasitems:
                        if self.take_money(i.price, "Items"):
            ...
            pool = list(i for i in pool if i not in newpool)
            shuffle(pool)
            # Items that remain is what a girl got to choose from.
            if dice(30):
                # 30% chance for her to simply buy any good restore item:
                newpool = list(item for item in pool if item.type == "restore")
                for i in newpool:
                    if dice(100 - i.badness) and self.take_money(i.price, "Items"):
                        self.inventory.append(i)
                        returns.append(i.id)
                        amount -= 1
                    # This will break for loop prematurely:
                    if not amount:
                        return returns 
            # Oki... so we got here...
            ...
            for inv in source:
                ...
                for item in content:
                    ...
                    for skill, effect in item.mod_skills.iteritems():
                        if skill in self.SKILLS: # Not usefull after we finish stats/skills?
                            ...
                            for i in effect[:3]:
                                if i >= 0:
                                    bonus = bonus + i*50 # Not sure if 50 is a good number here...
                                else:
                                    if skill in exclude_on_skills:
                    ...
                    d[item.id] = bonus + item.eqchance * 2 + possible_bonus - penalty
                ...
                if slot == "consumable":
                    ...
                    for skill in target_skills:
                        ...
                        # We do not need a complicated loop as with consumables, simply get the best item and equip it:
                        item = items[l[0]]
                        self.equip(item)
                        returns.append(item.id)
    ...
    class Girl(PytCharacter):
        ...
        def show(self, *tags, **kwargs):
            ...
            # select, load and resize image (Get imgpath)
            if type == "normal":
                # Try the mood first:
                if add_mood:
                    imgpath = self.select_image(*tags_with_mood, exclude=exclude)
                ...
                # Try with mood first...
                if add_mood:
                    if not imgpath and not len(tags) <= 3:
                        tags_backup = tags * 1
                        tags.remove(self.id)
                        main_tag = tags.pop(0)
                        while tags and not imgpath:
                            descriptor_tag = tags.pop()
                            imgpath = self.select_image(main_tag, descriptor_tag, mood_tag, self.id, exclude=exclude)
                        tags = tags_backup
                ...
                if add_mood:
                    while tags and not imgpath:
                        tag = tags.pop()
                        imgpath = self.select_image(tag, self.id, mood_tag, exclude=exclude)
                    tags = tags_backup
                ...
                if not imgpath and len(tags) > 2:
                    ...
                    if add_mood:
                        ...
                        tags = tags_backup
                    # "first_default" behavior OR auto-mood was not desired    
                    if not imgpath:
                        main_tag = tags.pop(0)
                        while tags and not imgpath:
                            descriptor_tag = tags.pop()
            ...
            # If we got here without being able to find an image:        
            if not imgpath:
