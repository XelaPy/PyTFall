...
screen pyt_girlslist(source=None, page=0, total_pages=1):
    frame:
        ...
        if girls:
            ...
            # Keybinds:
            key "mousedown_4" action If(gs["page"] + 1 < gs["total_pages"], true=Show("pyt_girlslist", source=gs["source"], page=gs["page"] + 1, total_pages=gs["total_pages"]), false=NullAction())
            key "mousedown_5" action If(gs["page"] > 0, true=Show("pyt_girlslist", source=gs["source"], page=gs["page"] - 1, total_pages=gs["total_pages"]), false=NullAction())
