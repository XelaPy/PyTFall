label fg_management:
    ...
    python:
        ...
        if global_flags.flag("teams_changed"):
            ...
            renpy.show_screen("pyt_message_screen", "Your teams setup has been changed, due to girls leaving your service or dieing!")
            
    $ fg_drags = FG_Drags()

    # $ renpy.retain_after_load()
    show screen pyt_fg_management
    with fade
            ...
            if isinstance(result, list):
                if result[0] == 'control':
                    ...
                    if result[1] == "add":
                        if hero.take_money(fg.get_team_price(), "Fighers Guild"):
                            ...
                            if team_name in list(team.name for team in fg.teams):
                                renpy.call_screen("pyt_message_screen", "Team name must be unique!")
                            else:
                                team = Team(name=team_name, max_size=3)
                                fg.teams.append(team)
                                fg.rooms += 3
                        else:
                            renpy.call_screen("pyt_message_screen", "You do not have the required funds (%d)!" % fg.get_team_price())
                    ...
                    if result[1] == "upgrade":
                        ...
                        if renpy.call_screen("yesno_prompt", message="Are you sure you wish to buy %s for %d Gold?" % (string.capwords(result[2]), upgrade[2]),
                            ...
                            if hero.take_money(upgrade[2], "Upgrades"):
                                upgrade[0] = True
                                renpy.call_screen("pyt_message_screen", "%s added to the Guild!" % string.capwords(result[2]))
                            else:
                                renpy.call_screen("pyt_message_screen", "Not enough money...")
                    ...
                    if result[1] == "area_pick":
                        ...
                        renpy.show_screen("pyt_area_pick", ua, pos=pos)
                    if result[1] == "gen_area_pick":
                        ...
                        for area in fg_areas.values():
                            if area.area in list(a.area for a in ua):
                                ...
                                ua.append(area)    
                        ua.sort(key=attrgetter("id"))
                        fg.focus_gen_area = renpy.call_screen("pyt_gen_area_pick", ua, pos=pos)
                    ...
                    if result[1] == "girl_menu":
                        pos = renpy.get_mouse_pos()
                        renpy.show_screen("pyt_fg_girl_menu", result[2], team=result[3], remove=result[4], pos=pos)
                                              
    hide screen pyt_fg_management
    jump mainscreen
...
screen pyt_explorer: ### Gismo: Just example ###
    modal True
    zorder 1
    frame:
        pos (223, 42)
...
screen pyt_log1: ### Gismo: Just example ###
    modal True
    zorder 1
    default tt = Tooltip("")
